...
private:
    ...
    void copy(const FMI& other) {
        ...
        for (std::size_t index{ 0 }; index < other.m_size; ++index) {
            if (typeid(*other.m_fmi[index]) == typeid(Lecturer)) {
                Lecturer* tempPtr = dynamic_cast<Lecturer*>(other.m_fmi[index]);
                
                this->m_fmi[index] = new Lecturer(*tempPtr);
            } else if (typeid(*other.m_fmi[index]) == typeid(Assistant)) {
                Assistant* tempPtr = dynamic_cast<Assistant*>(other.m_fmi[index]);
                
                this->m_fmi[index] = new Assistant(*tempPtr);
            } else if (typeid(*other.m_fmi[index]) == typeid(Student)) {
                Student* tempPtr = dynamic_cast<Student*>(other.m_fmi[index]);

                this->m_fmi[index] = new Student(*tempPtr);
            } else {
