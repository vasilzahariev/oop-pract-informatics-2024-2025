...
public:
    ...
    virtual FMIPerson* clone() {
        return new FMIPerson(this->m_name, this->m_age);
    }
...
public:
    ...
    virtual FMIPerson* clone() override final {
        return new Lecturer(this->m_name, this->m_age, this->m_subject, this->m_yearsOfExperience);
    }
...
public:
    ...
    virtual void whoAmI() const {
        ...
        std::string assistant = (this->m_isPartTime) ? "part-time" : "full-time";
        std::cout << "I am a " << assistant << " assistant in FMI and I teach " 
        << this->m_subject << std::endl;
    }

    virtual FMIPerson* clone() override final {
        return new Assistant(this->m_name, this->m_age, this->m_subject, this->m_isPartTime);
    }
...
public:
    ...
    virtual FMIPerson* clone() override final {
        return new Student(this->m_name, this->m_age, this->m_major, this->m_year, this->m_group);
    }
...
private:
    ...
    void copy(const FMI& other) {
        ...
        for (int index{ 0 }; index < other.m_size; ++index) {
            this->m_fmi[index] = other.m_fmi[index]->clone();
        }
